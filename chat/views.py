...
def index_view(request):
    return render(request, 'ItHasNavigationBar.html')
...
def detect_intent_with_parameters(project_id, session_id, query_params, language_code, user_input):
    ...
    if 'name' in query:
        data['name']=response.query_result.query_text
    elif 'age' in query:
        data['age']=response.query_result.query_text
    elif 'your address' in query:
        data['address']=response.query_result.query_text
    elif 'date of occurrence' in query:
        data['date']=response.query_result.query_text
    elif 'time' in query:
        data['time']=response.query_result.query_text
    elif 'description' in query:
        data['description']=response.query_result.query_text
    else:
        pass
    print(data)
